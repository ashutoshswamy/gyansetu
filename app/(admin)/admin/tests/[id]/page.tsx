  ...
  return (
    <div className="min-h-screen p-4 sm:p-8" style={{ background: "var(--background)" }}>
      <div className="max-w-6xl mx-auto">
        ...
        {/* Test Header */}
        <div className="rounded-xl p-6 mb-6 bg-white border" style={{ borderColor: "var(--border)" }}>
          <span style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", fontWeight: 600, color: "var(--gs-muted)" }}>
            Admin Console &middot; Test Details
          </span>
          <h1 className="mt-1" style={{ fontSize: 24, fontWeight: 700, color: "var(--foreground)", margin: "4px 0 6px 0" }}>
            {test.title}
          </h1>
          {test.description && (
            <p style={{ fontSize: 14, color: "var(--gs-text-secondary)", margin: "0 0 12px 0" }}>{test.description}</p>
          )}
          <div className="flex flex-wrap gap-x-6 gap-y-2 mt-4 pt-4 border-t text-[var(--gs-text-secondary)]" style={{ fontSize: 13, borderColor: "var(--gs-card)" }}>
            ...
            <span><strong>Status:</strong> <span style={{ textTransform: "capitalize", fontWeight: 600, color: test.status === "active" ? "var(--gs-success)" : "var(--gs-warning)" }}>{test.status}</span></span>
          </div>
        </div>
