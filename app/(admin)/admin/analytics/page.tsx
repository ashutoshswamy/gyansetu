    ...
    <div className="min-h-screen p-4 sm:p-8" style={{ background: "var(--background)" }}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, color: "var(--foreground)", margin: 0 }}>Analytics</h1>
          <p style={{ fontSize: 14, color: "var(--gs-text-secondary)", marginTop: 4 }}>Platform-wide statistics</p>
        </div>
            ...
            {(["pending", "shortlisted", "selected", "rejected"] as const).map((status) => {
              ...
              return (
                <div key={status} className="flex items-center gap-3 mb-3">
                  <span style={{ fontSize: 13, color: "var(--gs-text-secondary)", width: 80, textTransform: "capitalize", flexShrink: 0 }}>{status}</span>
                  <div style={{ flex: 1, height: 6, background: "var(--gs-card)", borderRadius: 3, overflow: "hidden" }}>
                    <div style={{ width: `${pct}%`, height: "100%", background: "var(--gs-accent)", borderRadius: 3 }} />
                  </div>
                  <span style={{ fontSize: 13, color: "var(--foreground)", width: 28, textAlign: "right", flexShrink: 0 }}>{count}</span>
                </div>
              );
            })}
          </CardContent>
