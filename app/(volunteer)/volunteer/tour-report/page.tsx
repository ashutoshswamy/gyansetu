      ...
      {Array.from({ length: 10 }, (_, i) => i + 1).map(n => (
        <button
          ...
          style={{
            ...
            border: value === n ? "1.5px solid var(--gs-success)" : "1.5px solid var(--border)",
            background: value === n ? "var(--gs-success)" : "var(--background)",
            color: value === n ? "white" : "var(--gs-text-secondary)",
            cursor: "pointer",
          }}
        >
          {n}
    ...
    <div className="min-h-screen p-4 sm:p-8" style={{ background: "var(--background)" }}>
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, color: "var(--foreground)", margin: 0 }}>Tour Report</h1>
        ...
        <form onSubmit={handleSubmit} style={{ background: "white", border: "1px solid var(--border)", borderRadius: 12, padding: 24, marginBottom: 24 }}>
          ...
          <h2 style={{ fontSize: 13, fontWeight: 700, color: "var(--gs-success)", margin: "0 0 12px", textTransform: "uppercase", letterSpacing: "0.06em" }}>1. Location Details</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            ...
            <div>
              <label style={{ fontSize: 12, fontWeight: 600, color: "var(--gs-text-secondary)", display: "block", marginBottom: 6 }}>Location Visited <span style={{ color: "var(--gs-danger)" }}>*</span></label>
          ...
          <h2 style={{ fontSize: 13, fontWeight: 700, color: "var(--gs-success)", margin: "0 0 12px", textTransform: "uppercase", letterSpacing: "0.06em" }}>2. Local Organisations & Host Details</h2>
          <div className="space-y-4 mb-3">
            {hosts.map((h, i) => (
              <div key={i} style={{ border: "1px solid var(--border)", borderRadius: 8, padding: 16, position: "relative" }}>
                {hosts.length > 1 && (
                  <button type="button" onClick={() => setHosts(prev => prev.filter((_, idx) => idx !== i))} style={{ position: "absolute", top: 10, right: 10, background: "none", border: "none", cursor: "pointer", color: "var(--gs-muted)" }}>
                    <X size={16} />
                  </button>
                )}
          ...
          <button type="button" onClick={() => setHosts(prev => [...prev, { ...EMPTY_HOST }])} className="flex items-center gap-1.5" style={{ fontSize: 12, fontWeight: 600, color: "var(--gs-accent)", background: "none", border: "none", cursor: "pointer", marginBottom: 24 }}>
            <Plus size={14} /> Add another host / organisation
          </button>

          {/* Section 3: Logistics Rating */}
          <h2 style={{ fontSize: 13, fontWeight: 700, color: "var(--gs-success)", margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.06em" }}>3. Logistics Rating</h2>
          <p style={{ fontSize: 12, color: "var(--gs-muted)", margin: "0 0 12px" }}>Rate each parameter from 1 (Poor) to 10 (Excellent)</p>
          <div className="space-y-3 mb-6">
            {LOGISTICS_FIELDS.map(f => (
              <div key={f.key} className="flex items-center justify-between gap-4 flex-wrap">
                <label style={{ fontSize: 13, color: "var(--gs-text-secondary)", minWidth: 200 }}>{f.label}</label>
          ...
          <h2 style={{ fontSize: 13, fontWeight: 700, color: "var(--gs-success)", margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.06em" }}>4. Observations</h2>
          <p style={{ fontSize: 12, color: "var(--gs-muted)", margin: "0 0 12px" }}>Minimum 50 words each (except where marked optional)</p>
              ...
              return (
                <div key={f.key}>
                  <div className="flex items-center justify-between" style={{ marginBottom: 6 }}>
                    <label style={{ fontSize: 12, fontWeight: 600, color: "var(--gs-text-secondary)" }}>
                      {f.label} {f.required && <span style={{ color: "var(--gs-danger)" }}>*</span>}
                    </label>
                    {f.required && (
          ...
          <div className="space-y-4 mb-2">
            <div>
              <label style={{ fontSize: 12, fontWeight: 600, color: "var(--gs-text-secondary)", display: "block", marginBottom: 6 }}>Overall Recommendation</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: "var(--gs-text-secondary)", display: "block", marginBottom: 6 }}>Suitable for Residential Camps</label>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: "var(--gs-text-secondary)", display: "block", marginBottom: 6 }}>Follow-up Required</label>
