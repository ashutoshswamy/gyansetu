...
export default async function EarcDocumentsPage() {
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
          <div>
            ...
            <h1 style={{ fontSize: 24, fontWeight: 700, color: "#19140F", margin: 0 }}>Documents</h1>
            <p style={{ fontSize: 14, color: "#5A5247", marginTop: 4 }}>
              Upload and manage general documents.
            </p>
          </div>
          <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: "rgba(184,56,30,0.08)" }}>
            <FolderOpen className="w-5 h-5" style={{ color: "#B8381E" }} />
          </div>
        </div>

        <EarcUploadForm category="document" accentColor="#B8381E" />
        <EarcFileList files={files ?? []} accentColor="#B8381E" />
      </div>
    </div>
  );
}
